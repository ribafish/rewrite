...
public class ScalaParserTest {
    ...
    void parseSimpleForLoop() {
        ...
        // Verify the for loop is parsed as a rich J.ForEachLoop (not J.Unknown)
        // Note: printTrimmed() uses Java-style printing so we verify the AST type instead
        J.Block methodBody = objectBody.getStatements().stream()
            .filter(s -> s instanceof J.MethodDeclaration)
            .map(s -> ((J.MethodDeclaration) s).getBody())
            .findFirst().orElse(null);
        assertThat(methodBody).isNotNull();
        boolean hasForEachLoop = methodBody.getStatements().stream()
            .anyMatch(s -> s instanceof J.ForEachLoop);
        assertThat(hasForEachLoop)
            .as("Method body should contain a J.ForEachLoop")
            .isTrue();
    }
