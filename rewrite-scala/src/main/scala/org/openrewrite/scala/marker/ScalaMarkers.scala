...
/**
 * Marks elements that use Scala 3 colon syntax instead of braces.
 * For example: `numbers.map: x => x * 2` instead of `numbers.map { x => x * 2 }`
 */
case class ColonToken(id: UUID) extends Marker {
  override def getId(): UUID = id
  override def withId[M <: Marker](newId: UUID): M = copy(id = newId).asInstanceOf[M]
}